...
def load_images(folder_path, resize_shape=None):
    ...
    for filename in filenames:
        ...
        img = PILImage.open(os.path.join(folder_path, filename))
...
@hydra.main(
    ...
    config_name="label_sim_kitchen_dataset_hf",
)
def label_dataset(cfg: DictConfig):
    ...
    if hasattr(cfg, 'use_huggingface') and cfg.use_huggingface:
        ...
        for demo_type in datasets:
            ...
            for sample_idx, sample in tqdm(enumerate(hf_dataset), disable=not cfg.verbose):
                # Get the video ID
                
                # Create save folder
                if demo_type != 'robot':
                    save_folder = os.path.join(
                        cfg.exp_path, f"{demo_type}_encode_protos", f"ckpt_{cfg.ckpt}", str(sample_idx)
                    )
                else:
                    save_folder = os.path.join(
                        cfg.exp_path, "encode_protos", f"ckpt_{cfg.ckpt}", str(sample_idx)
                    )
