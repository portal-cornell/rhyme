...
# import numpy as np
# import torch
# from rhyme.utility.file_utils import get_subdirs, get_files
# import random
# import collections
# import os
...
# from tqdm import tqdm
# import cv2
# from rhyme.utility.eval_utils import gif_of_clip
...
from rhyme.utility.eval_utils import gif_of_clip
from omegaconf import DictConfig
from datasets import load_dataset, Image  # Add import for Huggingface datasets
...
class KitchenBCDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        obs_horizon,
        action_horizon,
        dataset_name='prithwishdan/RHyME',  # Replace data_dirs with dataset_name
        config_name='robot',                # Specify the config name
        split='train',                      # Specify the split
        resize_shape=None,
        proto_dirs=None,                   # Keep proto_dirs for prototype data
        proto_horizon=None,
        raw_representation=False,
        softmax_prototype=False,
        prototype=False,
        ...
        """
        Modified KitchenBCDataset to use Huggingface datasets for actions and states
        """
        self.verbose = verbose
        self.resize_shape = resize_shape
        ...
        # Replace data_dirs with Huggingface dataset parameters
    ...
    def _load_huggingface_dataset(self):
        ...
        # Load dataset from huggingface
        self.hf_dataset = load_dataset(self.dataset_name, self.config_name, split=self.split)
        print(f"Loaded dataset {self.dataset_name}/{self.config_name} split {self.split} with {len(self.hf_dataset)} samples")
    ...
    def load_action_and_to_tensor(self, sample_idx):
        """Extract actions from Huggingface dataset sample"""
        sample = self.hf_dataset[sample_idx]
        
        # Convert actions from sample to numpy array
        # Assuming actions are stored in the sample
        action_data = np.array(sample["actions"], dtype=np.float32)
        
        return action_data

    def load_state_and_to_tensor(self, sample_idx):
        """Extract states from Huggingface dataset sample"""
        sample = self.hf_dataset[sample_idx]
        
        # Convert states from sample to numpy array
        # Assuming states are stored in the sample
        state_data = np.array(sample["states"], dtype=np.float32)
    ...
    def load_images(self, sample_idx):
        """
        Simple implementation to load images from the Huggingface dataset
        """
        if not self.obs_image_based:
            return None
        ...
        for frame_idx in range(len(frames_encoded)):
            # Decode the frame using the image decoder from datasets
            image_decoder = Image()
            ...
            # Resize if needed
            if self.resize_shape is not None:
                frame_np = cv2.resize(frame_np, tuple(self.resize_shape))
    ...
    def load_data(self, train_data):
        """
        Modified to use the Huggingface dataset for loading data
        """
        assert not (self.paired_data and self.nearest_neighbor_replacement)
        print("loading data")
        ...
        paired_set = set()
        if self.paired_data:
            sample_indices = list(range(len(self.hf_dataset)))
            np.random.shuffle(sample_indices)
            paired_set.update(sample_indices[:int(len(self.hf_dataset) * self.paired_percent)])

        lookup_set = set()
        if self.nearest_neighbor_replacement:
            sample_indices = list(range(len(self.hf_dataset)))
            np.random.shuffle(sample_indices)
            lookup_set.update(sample_indices[:int(len(self.hf_dataset) * self.replace_percent)])
        
        for sample_idx in tqdm(range(len(self.hf_dataset)), desc="Loading data", disable=not self.verbose):
            
            # Get the video_id from the Huggingface dataset sample
            video_id = self.hf_dataset[sample_idx]["video_id"]
                
            if self.obs_image_based:
                images = self.load_images(sample_idx)
                if images is not None:
                    train_data["images"].append(images)

            # Load states and actions from the Huggingface dataset
            train_data["obs"].append(self.load_state_and_to_tensor(sample_idx))
