...
from rhyme.model.common.dict_of_tensor_mixin import DictOfTensorMixin
...
class LinearNormalizer(DictOfTensorMixin):
    ...
    def __call__(self, x: Union[Dict, torch.Tensor,
    ...
    def normalize(self, x: Union[Dict, torch.Tensor,
                                 np.ndarray]) -> torch.Tensor:
        return self._normalize_impl(x, forward=True)

    def unnormalize(self, x: Union[Dict, torch.Tensor,
                                   np.ndarray]) -> torch.Tensor:
...
class SingleFieldLinearNormalizer(DictOfTensorMixin):
    ...
    @classmethod
    def create_fit(cls, data: Union[torch.Tensor, np.ndarray, zarr.Array],
                   **kwargs):
        obj = cls()
        obj.fit(data, **kwargs)
        return obj
    ...
    @classmethod
    def create_manual(cls, scale: Union[torch.Tensor, np.ndarray],
                      offset: Union[torch.Tensor, np.ndarray],
                      input_stats_dict: Dict[str, Union[torch.Tensor,
                                                        np.ndarray]]):
        ...
        params_dict = nn.ParameterDict({
            ...
            to_tensor(offset),
            'input_stats':
            nn.ParameterDict(dict_apply(input_stats_dict, to_tensor))
        })
...
def _fit(data: Union[torch.Tensor, np.ndarray, zarr.Array],
    ...
    if mode == 'limits':
        if fit_offset:
            ...
            offset[ignore_dim] = (output_max +
                                  output_min) / 2 - input_min[ignore_dim]
            ...
            input_abs = torch.maximum(torch.abs(input_min),
                                      torch.abs(input_max))
