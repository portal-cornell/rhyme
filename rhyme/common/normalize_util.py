from rhyme.model.common.normalizer import SingleFieldLinearNormalizer
from rhyme.common.pytorch_util import dict_apply, dict_apply_reduce, dict_apply_split
import numpy as np


def get_range_normalizer_from_stat(stat,
                                   output_max=1,
                                   output_min=-1,
                                   range_eps=1e-7):
    # -1, 1 normalization
...
def robomimic_abs_action_normalizer_from_stat(stat, rotation_transformer):
    ...
    param = dict_apply_reduce([pos_param, rot_param, gripper_param],
                              lambda x: np.concatenate(x, axis=-1))
    info = dict_apply_reduce([pos_info, rot_info, gripper_info],
                             lambda x: np.concatenate(x, axis=-1))
...
def robomimic_abs_action_only_normalizer_from_stat(stat):
    ...
    param = dict_apply_reduce([pos_param, other_param],
                              lambda x: np.concatenate(x, axis=-1))
    info = dict_apply_reduce([pos_info, other_info],
                             lambda x: np.concatenate(x, axis=-1))
...
def robomimic_abs_action_only_dual_arm_normalizer_from_stat(stat):
    ...
    param = dict_apply_reduce(
        ...
        lambda x: np.concatenate(x, axis=-1))
    info = dict_apply_reduce([pos0_info, other0_info, pos1_info, other1_info],
                             lambda x: np.concatenate(x, axis=-1))
